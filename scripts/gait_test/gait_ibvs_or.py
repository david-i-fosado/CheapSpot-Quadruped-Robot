...
class BezierIKBridge:
    ...
    def gait_callback(self, msg):
        ...
        rospy.loginfo(f"X:{self.pitch_vel:.3f}")
        rospy.loginfo(f"Y:{self.pitch_vel:.3f}")
        rospy.loginfo(f"Z:{self.yaw_vel:.3f}")
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Resolver IK
            joint_angles = self.spot.IK([self.roll_cmd, self.pitch_cmd, self.yaw_cmd], [0, 0, 0], self.T_bf)
