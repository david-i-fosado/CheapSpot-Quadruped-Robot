...
from std_msgs.msg import Float32MultiArray # Mensaje ROS que contiene una lista de flotantes
...
def enviar_arduino(comandos):
    ...
    msg = '<' + ','.join(f"{c:.2f}" for c in comandos) + '>'
    rospy.loginfo(msg)
...
def listener():
    ...
    rospy.Subscriber('/servo_positions', Float32MultiArray, callback) # se Subscribe
